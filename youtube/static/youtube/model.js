...
(function (root, factory) {
  ...
  function WordFilter(api, parent, def) {
    ...
    this._id =
      typeof def === "undefined" ? "" : "id" in def ? "" + def["id"] : "";
    this._phrase = typeof def === "undefined" ? "" : def["phrase"];
    this._case_sensitive =
      typeof def === "undefined" ? "" : def["case_sensitive"];
    this._spell_variants =
      typeof def === "undefined" ? "" : def["spell_variants"];
    this._phrase_regex = typeof def === "undefined" ? "" : def["phrase_regex"];
    this._num_matched_comments =
      typeof def === "undefined" ? 0 : def["num_matched_comments"];
    // this._rule_variant = (typeof def == 'undefined') ?
    //   "DN" : def['rule_variant'];
    // this._num_matched_comments =
    //   typeof def === "undefined" ? 0 : def["num_matched_comments"];
  }
  ...
  WordFilter.prototype.finalize = function () {
    if (this.isFinalized()) {
      return Promise.reject(
        new Error("Cannot finalize an already finalized rule")
      );
    } else {
      return this._api
        .updateFilter(this._parent.getId(), "rules:add", this.serialize())
        .execute()
        .then(
          function (rule) {
            this._id = rule.id;
            ...
            this._rule_variant = rule.rule_variant;
            // this._action = rule
          }.bind(this)
        );
    }
  ...
  WordFilter.prototype.setPhrase = function (phrase) {
    if (!this.isFinalized()) {
      ...
      this._phrase = phrase;
      return this._api
        .updateFilter(this._parent.getId(), "rules:modify", this.serialize())
        .execute();
    }
  };
  ...
  WordFilter.prototype.setCaseSensitive = function () {
    if (!this.isFinalized()) {
      ...
      return this._api
        .updateRule(this.getId(), "toggle_case_sensitive")
        .execute()
        .then(
          function (serialized) {
  ...
  WordFilter.prototype.setSpellVariants = function () {
    // WordFilter can be in a state where it has not been uploaded to server
    // When creating, server doesn't know anything about the WordFilter (hasn't
    // been finalized). Modifying purely front-end rep. If class not finalized,
    // means we are creating a new word filter. While it is being created, I
    // can't call the methods to update backend/server yet so just update local
    // in meantime.
    if (!this.isFinalized()) {
      ...
      return this._api
        .updateRule(this.getId(), "toggle_spell_variants")
        .execute()
        .then(
          function (serialized) {
            this._id = serialized.id;
  ...
  // WordFilter.prototype.setRuleAction = function (rule) {
  //   alert(rule);
  // Check state first (has this rule been created in db yet)
  // if yes, send api request to update it otherwise update local

  // if (!this.isFinalized()) {
  //   this._rule_variants = rule;
  //   return Promise.resolve();
  // } else {
  //   return this._api
  //     .updateRule(this.getId(), "toggle_spell_variants")
  ...
  //         this._id = serialized.id;
  //         this._spell_variants = serialized.spell_variants;
  //         return this._rule_variants;
  //       }.bind(this)
  //     );
  // }
  // }

  // WordFilter.prototype.getRuleAction = function () {
  //   return this._rule_variants;
  // };
  ...
  WordFilter.prototype.preview = function (numExamples) {
    return this._api
      .getRulePreview(this._parent.getId(), this.serialize(), numExamples)
      .execute();
  };
  ...
  WordFilterGroup.prototype.finalizePreviewRule = function () {
    return this._previewRule.finalize().then(
      function () {
  ...
  WordFilterGroup.prototype.removeRule = function (rule) {
    ...
    return this._api
      .updateFilter(this._id, "rules:remove", rule.serialize())
      .execute()
      .then(
        function () {
  ...
  WordFilterGroup.prototype.setName = function (newName) {
    if (this.isFinalized()) {
      return this._api
        .updateFilter(this._id, "name", newName)
        .execute()
        .then(
          function () {
            this._name = newName;
  ...
  WordFilterGroup.prototype.finalize = function (reference) {
    ...
    return this._api
      .createFilter(this._name, reference)
      .execute()
      .then(
        function (serialized) {
          this._id = serialized.id;
  ...
  WordFilterGroup.prototype.delete = function () {
    if (this.isFinalized()) {
      return this._api
        .removeFilter(this._id)
        .execute()
        .then(
          function () {
            var id = this._id;
            this._parent._setGroups(
              this._parent._filterGroups.filter(function (g) {
                return g.getId() !== id;
              })
            );
          }.bind(this)
