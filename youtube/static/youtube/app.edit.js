...
(function (root, factory) {
  ...
  function (
    ...
    TabManager.prototype.toggle = function (names) {
      if (typeof names === "string") {
        this._tabs[names].style.display =
          this._tabs[names].style.display === "none" ? "" : "none";
      } else if (Array.isArray(names)) {
        names.forEach(
          function (name) {
            this.toggle(name);
    ...
    App.prototype._bind = function (userInfo) {
      ...
      // Setup the tables
      this._tablePreview = new InteractiveTable(
        $("table-preview"),
        ["actions", "comment", "author", "video", "time", "caught_by"],
        function (src, col) {
          if (col === "actions") {
            return null;
          } else if (col === "comment") {
      ...
      this._tableRules = new InteractiveTable(
        $("table-rules"),
        [
          "actions",
          "phrase",
        ...
        function (src, col) {
          if (col === "actions") {
            ...
            delButton.addEventListener(
              ...
              function (e) {
                ...
                src
                  ...
                  .then(
                    function () {
                      this._tableRules.removeRows(function (r) {
                        return r === src;
                      });
            ...
            checkboxButton.addEventListener(
              ...
              function (e) {
                ...
                src
                  ...
                  .catch(function (e) {
                    alert(
                      "Update case sensitive checkboxButton for rule failed with: " +
                        e
                    );
                  });
              }.bind(this)
            ...
            checkboxSpellButton.addEventListener(
              "click",
              function (e) {
                e.preventDefault();
                src
                  ...
                  .catch(function (e) {
                    alert(
                      "Update spell variants checkboxButton for rule failed with: " +
                        e
                    );
                  });
              }.bind(this)
            ...
            listItems.forEach(function (item) {
              item.addEventListener("click", function (e) {
                ...
                // call the server, drop-down menu was changed, update item in
                // models.py
                // right now, can id's to the list items (can add attributes to link
                // items to identify which got pressed)
                // src.setRuleAction(e.target.innerText)
                //   .then(
                // how do i handle the new rule variant in front end
                //     function (new_rule_variants) {
                //       if () {
                //
                //       } else if () {
                //
                ...
                //   .catch(function (e) {
                //     alert(
                //       "Update rule variants checkboxButton for rule failed with: " +
                //         e
                //     );
                //   });
              });
      ...
      this._P.listen(
        "gui.filter-name.change",
        function (e) {
          var currentFilter = this._currentGroup;
          return currentFilter.setName(e.target.innerText).then(
            function () {
      ...
      this._P.listen(
        "gui.rule.preview.change",
        function (e) {
          var currentFilter = this._currentGroup;
          if (currentFilter === null) {
            throw new Error(
              "Illegal state. Cannot preview rule with no filter."
            );
          }
          // Get the phrase
        ...
        function (filter) {
          if (filter.previewRule().getPhrase().length > 0) {
            return filter
              ...
              .then(
                function (comments) {
                  $("label-preview-mode").innerHTML =
                    'The table below shows a sample of comments that are matched by the phrase "' +
                    filter.previewRule().toString() +
                    '"' +
                    '. <span style="background-color:yellow;">Adding this rule will catch <b>' +
                    comments["num_new_matches"].toString() +
                    "</b> new comments. </span>";
      ...
      this._P.listen(
        ...
        function (e) {
          ...
          return currentFilter.finalizePreviewRule().then(
            function () {
      ...
      this._P.listen(
        ...
        function (item) {
          var currentFilter =
            typeof item !== "undefined" ? item : this._currentGroup;
          if (currentFilter !== null) {
